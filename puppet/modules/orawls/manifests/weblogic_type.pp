...
define orawls::weblogic_type (
  ...
  $jdk_home_dir         = undef, # /usr/java/jdk1.7.0_nn
  ...
  if ( $filename == undef or $oracle_base_home_dir == undef or $middleware_home_dir == undef or $jdk_home_dir == undef or $os_user == undef or $os_group == undef or $download_dir == undef ) {
    fail("please provide all the required parameters (file ${filename} base ${oracle_base_home_dir} mw ${middleware_home_dir} jdk ${jdk_home_dir} user ${os_user} group ${os_group} download ${download_dir})")
  }
